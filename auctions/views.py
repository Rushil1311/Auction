...
from .models import Category, Feedback, Payment, User, Listing, Watch, Bid, Comment
...
def register(request):
    if request.method == 'POST':
        ...
        if not re.search(r"[a-z]", password):
            password_errors.append('Password must include at least one lowercase letter.')
        if not re.search(r"\d", password):
            password_errors.append('Password must include at least one digit.')
...
def password_otp(request):
    if request.method == 'POST':
        ...
        if otp == request.session.get('otp', 0):
            ...
            password = get_random_string(length=12)  # Generates a 12-character random password
            user = User.objects.get(username=username, email=email)
...
def password_reset_success(request):
    return render(request, 'auctions/authentication/password_reset_succesful.html')
...
def feedback_page(request):
    return render(request, 'auctions/feedback.html')
...
def aview_profile(request):
    return render(request,'auctions/admin/view-profile.html')

def view_feedback(request):
    data = Feedback.objects.all()
    context = {'data':data}
    return render(request,'auctions/admin/view-feedback.html',context)
...
def manage_category(request):
    ...
    return render(request,'auctions/admin/manage-category.html', {'categories': categories})
...
def edit_category(request, category_id):
    ...
    return render(request, "auctions/admin/edit_category.html", {"category": category})
...
def initiate_payment(request, listing_id):
    ...
    amount = int(listing.starting_value * 100)  # Razorpay uses amount in paise
...
def payment_list(request):
    ...
    return render(request, "auctions/admin/payments_list.html", {"payments": payments})